...
int set_tmp_param_lid(vm_t *machine, champ_t *champ, int param, int *start)
{
    int res = 0;
...
void inst_ldi_reg_part(vm_t *machine, champ_t *champ, int tmp, int start)
{
    int start_two = (champ->prog_count + tmp % IDX_MOD) % MEM_SIZE;
    int tmp_two = champ->prog_count + tmp % IDX_MOD;
...
void do_ldi_inst(vm_t *machine, champ_t *champ, int *params)
{
    int start = (champ->prog_count + 2) % MEM_SIZE;
    int tmp = 0;

    champ->carry = false;
    tmp = set_tmp_param_lid(machine, champ, params[0], &start);
    while (start < 0)
        start += MEM_SIZE;
    start %= MEM_SIZE;
    tmp += set_tmp_param_lid(machine, champ, params[1], &start);
    while (start < 0)
        start += MEM_SIZE;
    start %= MEM_SIZE;
    inst_ldi_reg_part(machine, champ, tmp, start);
}
...
bool inst_ldi(vm_t *machine, champ_t *champ)
{
    ...
    if (!inst_parms_verif(op, params))
        return (false);
    do_ldi_inst(machine, champ, params);
