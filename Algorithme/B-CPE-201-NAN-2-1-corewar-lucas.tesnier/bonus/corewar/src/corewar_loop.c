...
int check_all_champs(vm_t *machine, int i)
{
    ...
    while (ptr) {
        count ++;
        if (ptr->last_live_cycle < (machine->max_cycle - machine->actual_cycle))
            ptr->living = false;
        if (!ptr->living)
            count --;
        ptr = ptr->child;
...
void corewar_champion_fight(vm_t *machine)
{
    ...
    machine->max_cycle += 1;
    if (machine->graph && machine->max_cycle % 100 == 0)
        draw_graph(machine, false);
...
void launch_virtual_machine(int ac, char **av)
{
    ...
    while (sfTime_asMilliseconds(sfClock_getElapsedTime(machine->window->time)) < 41000) {
