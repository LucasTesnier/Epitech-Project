...
bool or_param_zero(vm_t *machine, champ_t *champ, \
...
bool or_param_one(vm_t *machine, champ_t *champ, \
...
void do_or_inst(vm_t *machine, champ_t *champ, int *params)
{
    int start = (champ->prog_count + 2) % MEM_SIZE;
    unsigned char *val_one = my_calloc_u_char(REG_SIZE + 1, 0);
    unsigned char *val_two = my_calloc_u_char(REG_SIZE + 1, 0);

    if (params[0] == 1)
        if (!or_param_zero(machine, champ, start, val_one))
            return;
    set_start_from_param(machine, params[0], val_one, &start);
    if (params[1] == 1)
        if (!or_param_one(machine, champ, start, val_two))
            return;
    set_start_from_param(machine, params[0], val_one, &start);
...
bool inst_or(vm_t *machine, champ_t *champ)
{
    ...
    if (!inst_parms_verif(op, params))
        return (false);
    do_or_inst(machine, champ, params);
