...
export diff_size=0;
./../antman/antman ../tests/assets/file_three_1.ppm 0 > temp_file
./../giantman/giantman ../tests/temp_file > temp_file_two
diff=`cmp ./assets/file_three_1.ppm temp_file_two`
echo $diff
compress_size=`stat -c "%s" temp_file`
uncompress_size=`stat -c "%s" ../tests/assets/file_three_1.ppm`
