...
void shell_command_repartition(char **envp, old_commands_t *old_commands, char *command)
{
    int bultins = 0;
    int pos = 0;
    ...
    if (more_commands == true) {
        command += pos;
        shell_command_repartition(envp, old_commands, command);
    }
}

void shell_start(char **envp, old_commands_t *old_commands)
{
    int bultins = 0;
    char *command = NULL;
