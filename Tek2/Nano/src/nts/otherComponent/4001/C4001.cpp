...
/**
 * @brief Construct a new nts::other Component::C4001::C4001 object
 * 
 */

nts::otherComponent::C4001::C4001()
{
    nts::pinInfo temp;
...
bool nts::otherComponent::C4001::calcNorGate(nts::pinInfo pin1, nts::pinInfo pin2)
{
    if (pin1.pinState == nts::Tristate::FALSE && pin2.pinState == nts::Tristate::FALSE)
        return true;
    return false;
}
...
void nts::otherComponent::C4001::simulate(std::size_t tick)
{
...
/**
 * @brief compute the state of pin
 * 
 * @param pin 
 * @return nts::Tristate 
 */

nts::Tristate nts::otherComponent::C4001::compute(std::size_t pin)
{
    (void) pin;
    if (calcNorGate(pin_list[0], pin_list[1]))
        pin_list[2].pinState = nts::Tristate::TRUE;
    else
        pin_list[2].pinState = nts::Tristate::FALSE;
    
    if (calcNorGate(pin_list[4], pin_list[5]))
        pin_list[3].pinState = nts::Tristate::TRUE;
    else
        pin_list[3].pinState = nts::Tristate::FALSE;

    if (calcNorGate(pin_list[7], pin_list[8]))
        pin_list[9].pinState = nts::Tristate::TRUE;
    else
        pin_list[9].pinState = nts::Tristate::FALSE;
    
    if (calcNorGate(pin_list[11], pin_list[12]))
...
/**
 * @brief Destroy the nts::other Component::C4001::C4001 object
 * 
 */
